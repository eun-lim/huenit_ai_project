    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for idx, detection in enumerate(dr_data):
            ...
            if show_animation:
                ...
                '''
                output_img_path = os.path.abspath(output_files_path + "/images/detections_one_by_one/" + class_name + "_detection" + str(idx) + ".jpg")
                cv2.imwrite(output_img_path, img)
                success = cv2.imwrite(output_img_path, img)
                if not success:
                    print(f"Failed to save image at: {output_img_path}")
                    if img is None:
                        print(f"Image data is None for {output_img_path}")
